...
library PoolLibrary {
    ...
    function executeSwapForUser(
        ...
        if (tokenIn == address(0)) {
            ...
            amountOut = (amountIn * totalTokenBalances[tokenOut]) / (ethBalance + amountIn);
            require(amountOut >= minAmountOut, "Slippage too high");
            require(totalTokenBalances[tokenOut] >= amountOut, "Insufficient token liquidity");
            ...
            amountOut = (amountIn * ethBalance) / (totalTokenBalances[tokenIn] + amountIn);
            require(amountOut >= minAmountOut, "Slippage too high");
            ...
            require(tokenBalances[user][tokenIn] >= amountIn, "Insufficient token balance");
            require(lockedTokenBalances[user][tokenIn] >= amountIn, "Insufficient locked tokens");
            amountOut = (amountIn * totalTokenBalances[tokenOut]) / (totalTokenBalances[tokenIn] + amountIn);
            require(amountOut >= minAmountOut, "Slippage too high");
            require(totalTokenBalances[tokenOut] >= amountOut, "Insufficient token liquidity");
