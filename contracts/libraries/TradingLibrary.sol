...
library TradingLibrary {
  ...
  function executeOrder(
      ...
      OracleUpgradeable oracle,
      Order storage order,
      address executor
  ) external returns (uint256 amountOut, uint256 reward) {
      if (order.executed) revert OrderExecuted();
      if (!shouldExecuteOrder(oracle, order)) revert ExecutionConditionsNotMet();
  ...
  function shouldExecuteOrder(OracleUpgradeable oracle, Order memory order) public view returns (bool) {
      if (order.executed) return false;

      uint256 currentPrice = getCurrentPrice(oracle, order.tokenIn, order.tokenOut);
  ...
  function getCurrentPrice(OracleUpgradeable oracle, address tokenIn, address tokenOut) public view returns (uint256) {
