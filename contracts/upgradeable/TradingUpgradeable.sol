...
contract TradingUpgradeable is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
   ...
   function createLimitOrder(
       ...
       TradingLibrary.validateOrderCreation(address(oracle), user, amountIn, tokenIn, tokenOut);

       if (minAmountOut == 0) {
           minAmountOut = TradingLibrary.calculateMinAmountOut(address(pool), tokenIn, tokenOut, amountIn);
       }
   ...
   function modifyOrder(address user, uint256 orderId, uint256 newTargetPrice, uint256 newMinAmountOut) external {
       ...
       if (newMinAmountOut == 0) {
           newMinAmountOut = TradingLibrary.calculateMinAmountOut(address(pool), order.tokenIn, order.tokenOut, order.amountIn);
       }
   ...
   function getCurrentPrice(address tokenIn, address tokenOut) public view returns (uint256) {
       return TradingLibrary.getCurrentPrice(address(oracle), tokenIn, tokenOut);
   }

   function calculateMinAmountOut(address tokenIn, address tokenOut, uint256 amountIn) public view returns (uint256) {
       return TradingLibrary.calculateMinAmountOut(address(pool), tokenIn, tokenOut, amountIn);
   }
