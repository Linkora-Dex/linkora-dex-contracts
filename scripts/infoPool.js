...
console.log(`0.1 ETH → CAPY: ${ethers.formatUnits(ethToCapy, config.tokens.CAPY.decimals)} CAPY`);
} catch (error) {
console.log(`❌ ETH → CAPY swap calculation failed: ${error.message}`);
}

try {
const capyToEth = await router.getAmountOut(ethers.parseUnits("10", config.tokens.CAPY.decimals), config.tokens.CAPY.address, ethers.ZeroAddress);
